...
- TestEngineer 员工实例的具体协作关系和事项记录写入 support employee workspace 或 runtime cognition state。
- 正式测试职责、质量门禁和输入来源进入 `.agent.md`、test docs 或 workflow。
- TestEngineer 当前由 CTO acting 管理；正式向 CTO 报告，与 CTO 共同维护工程门禁。
...
- colleagues 层用于承载当前 TestEngineer 员工实例在测试工作层面的关系、协作偏好、事项记录和待确认信息。
