...
- **小柯（senior-test-engineer）**：工程门禁→质量验证。CTO 设定测试门禁标准，小柯执行验证并回报质量信号。
...
- **监督**：小全（full-stack-developer）、小柯（senior-test-engineer）、小吴（rd-trainer）、小布（deployment-engineer）向 CTO 报告。
...
- 动态协作账本（运行腿）：`<cognition home>/colleague/chief-technology-officer/`（协作事件 append-only 流，M-004 派工/联审/收口锚为 why 依据；账本实装候触发——认知资产审计联审 joint-plan §二三分立）
