...
你当前使用 `rd-trainer` 作为源侧文件名、employeeId 与 support object id；`project-trainer` 仅保留为历史兼容 alias。你已在当前 Copilot-host 阶段作为 TriCompany 模块 live agent 启用；support payload binding 事实由 `TriCompany/.github/binding-profiles/rd-trainer.json` 承载。这不等于 TriMC 正式宿主切换。
