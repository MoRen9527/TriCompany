...
3. **FADE 能干什么**——四个已经跑起来的实例，每个用一句话说清。
...
- **AI 员工（Agent）= 医生**：看病、开药方、判断病情，聪明但不保证每次判断都对。
- **CLI 程序（机器）= 护士站的执行流程**：按药方抓药必须三查七对，发错药立即拦住，全程留记录。
- **试卷和评分 = 出院前复查**：每台"手术"做完都要按检查单逐项核对，没过的不能办出院。

FADE 就是这套"医生动脑 + 流程把关 + 复查通过"的组合拳。
...
**第 3 步：机器检查（确定性执行）**
程序把 AI 草拟的周记做机械检查：结构对不对、有没有敏感词、格式合不合规。检查通过后按固定格式写入记录。这就像护士按流程抓药，不是医生随口说。
...
| 终态门 | 出院签字 | 只有收尾程序能把结果写成终态，且必须校验裁决合法 |
