...
版本：V1.1
日期：2026-08-07
状态：当前 DCE 可用教程；完整 ADE 生命周期待实现
维护人：RAndDTrainer（小吴）
适用对象：需要维护跨仓项目文档的研发、产品、技术与总助协作者

## 文档同步元信息

- sourceOfTruth: TriCompany/docs/training/project-source-document-sync-ade-tutorial.md
- syncMode: follow-spec（本教程讲 FADE-002 项目真源文档同步；规范真源 = `../workflow/project-source-document-sync-ade.md`，ADE 生命周期框架真源 = `../engineering/ade-pattern-spec.md`，教程随二者联动更新；spec 当前 source-only，未发布前本档仅存源侧）
- syncWith: docs/workflow/project-source-document-sync-ade.md, docs/engineering/ade-pattern-spec.md
...
4. 看懂 `pass`、`partial`、`fail` 三种 ADE 收口状态。
5. 在真源变化时，知道什么时候可以让 CLI 直接复制，什么时候必须由 Agent 先整理摘要候选。

当前成熟度是“DCE 可执行、ADE lifecycle 待补齐”：

- 已实现 manifest 驱动的跨仓文档检查。
- 已实现 `published-copy` 的确定性复制。
- 已实现 `published-summary` 的 source revision 校验和候选发布。
- 已实现 dry-run、显式写入、安全门、结构化 JSON 与回归测试。
- 当前未实现事件 watcher、持久 runId、Plan / Close Skill、Close CLI，也不处理 `audit-record`。
...
完整项目真源文档同步 ADE 使用下面这条链：

```text
事件或 Agent 检测 -> 程序登记 run
  -> Plan Skill（小贾）
  -> 小乔核产品语义 / 小狄核技术与安全门
...
| 阶段 | 负责人 | 做什么 | 当前状态 |
| --- | --- | --- |
| Event / run | Runtime | 登记事件、去重、生成 runId、恢复和重试 | 待实现 |
| Plan Skill | 小贾 | 确认真源、目标、同步模式、条目范围和验收标准 | 当前人工 |
...
| Close Skill | 小贾 | 读取证据，形成 APPROVE、FREEZE、ESCALATE 或 RETRY | 当前人工 |
| Close CLI | Runtime CLI | 校验裁决、推进终态并写审计 | 待实现 |
...
它尚不构成完整 ADE 生命周期，因为没有持久 run、Close Skill 装载和 Close CLI 终态提交。
...
当前可由小贾人工 APPROVE 本次 DCE 切片。完整 ADE 落地后，应由 Close Skill 先形成裁决，再由 Close CLI 校验并写入终态。
...
| --- | --- | --- |
| `pass` | 结合业务证据决定 APPROVE 或继续检查 | 校验裁决后写终态 |
| `partial` | RETRY 或 FREEZE | 保持非终态，记录补规划要求 |
| `fail` | FREEZE、ESCALATE 或 RETRY | 校验失败证据并落账 |
...
`_serialize_project_doc_sync_report()` 把运行结果变成 ADE JSON：
...
- 一个 canonical DCE CLI 下的独立项目文档模式。
...
- 自动生成摘要候选。
- 自动替代小乔、小狄的语义与技术联审。
- 文件 / Git 事件 watcher、去重和持久 runId。
- Plan Skill 与 Close Skill 的版本化装载。
- Close CLI 的状态校验、终态提交和审计落账。
...
1. ADE 总规范：[ADE 模式：Agent 智能任务确定性执行规范](../engineering/ade-pattern-spec.md)
2. 项目文档同步 workflow：[项目真源文档同步 ADE](../workflow/project-source-document-sync-ade.md)
3. 生命周期行业联审：[ADE 生命周期行业模式联审](../engineering/ade-lifecycle-industry-review.md)
4. 机器清单：[project-source-doc-sync-manifest.json](../../.github/manifests/project-source-doc-sync-manifest.json)
5. DCE 实现：[source_publish_check.py](../../runtime/cognition/source_publish_check.py)
6. 回归测试：[source_publish_check_validation.py](../../runtime/cognition/source_publish_check_validation.py)
7. 当前源侧宪章：[TriCompany 赛博公司宪章](../../tricompany.md)
8. 当前中央摘要：[TriCompany 赛博公司中央摘要](../../../TriMetaverse/tricompany.md)

## 17. 最后记住四句话

1. ADE 是完整生命周期，DCE 只是确定性执行阶段。
