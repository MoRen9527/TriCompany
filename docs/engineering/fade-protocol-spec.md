...
版本：v2.0.3（2026-08-28：试卷 Plan 时点冻结立法——§2.8 Plan 行+§2.6 联动，LG-008 三方联审；v2.0.2=细则 10 定级正式法条（87f16cd）；v2.0.0=架构重构 ADE 退役，FADE 升为协议本体，FADE-XXX 为实现）
...
- v2.0.3（2026-08-28，试卷 Plan 时点冻结立法·LG-008 三方联审）：§2.8 Plan 行扩容——试卷声明自"随附项"升格为**第三件 Plan 时点冻结件**（三不变量：Plan 时点冻结 / DCE 期间不可变 / 收口对卷，第三不变量在试卷语境=「评分对卷」）；§2.6 增补「试卷 Plan 时点冻结」条款（冻结对象=解析后整卷双 hash raw+LF，行尾漂移按 SOFT-DRIFT 留痕；冻结声明必含双门槛要素；试卷 hash 入 run root 引用集）；明文裁定**语义作业方案卷封不涵盖试卷**（防自改考卷，双席一致）。既有实例回溯建卷按历史冻结口径如实标注（FADE-006 增评卷先例）。**v2.0.2 补记**：87f16cd（细则 10 升正式法条+修正 1 纸面法清单齿条+修正 2 不溯及既往+FADE-006 双轨）落地时未同步本变更记录与版本号，随本版补记
...
**试卷 Plan 时点冻结（v2.0.3 立法，LG-008 三方联审）**：试卷是 Score 段的评分合同，评分不能引用一份执行期可被改动的基准——试卷自本版起为 **Plan 段第三件时点冻结件**：

1. **Plan 时点冻结**：试卷声明随 Plan 落盘，声明必含**双门槛要素**（检查项/权重＋及格线阈值）；
2. **DCE 期间不可变**：冻结对象=解析后整卷，双 hash raw+LF（与卷封制同口径，行尾漂移按 SOFT-DRIFT 留痕不按污染裁决）；
3. **收口对卷（试卷语境）**：评分段实际使用的试卷须与冻结卷一致——「评分对卷」；试卷 hash 入 run root 引用集。

语义作业方案卷封不涵盖试卷（两者段位不同，防自改考卷）；工具化按"最小交付"渐进（先 hash 留痕，后接入卷封工具族）。既有实例回溯建卷按历史冻结口径如实标注，不溯及改卷（FADE-006 增评卷先例）。
...
| Plan | 结构化计划（任务树分解＋两类卷封：原材料/语义作业方案）＋**试卷声明（第三件 Plan 时点冻结件，v2.0.3）**——三不变量：Plan 时点冻结 / DCE 期间不可变 / 收口对卷（试卷语境=「评分对卷」），细则见 §2.6 |
...
> **语义作业方案卷封**：Plan 阶段对每个工作项预封的智能处理方式与语义检查方法清单——"做完且做对"的语义判定基准；与原材料卷封（输入完整性基准）构成对偶。不变量：Plan 时点冻结、DCE 期间不可变、收口必须对卷。**不涵盖试卷**（v2.0.3，LG-008 双席一致：防"自改考卷"）——试卷按 §2.6 试卷冻结条款独立冻结，两者段位不同（前者 DCE 段语义基准，后者 Score 段评分合同）。
...
10. **立法完成度（v2.0.2 三方联审定级：正式法条）**：审出的缺陷写进法条只是及格线——**接线（执行路径真实存在）+ 实测（模拟或真跑证伪过）才算立法完成**。未接线的法条一律标注"纸面法"，登记于 fade-registry 周检节「纸面法清单」，每条必载**解除条件+责任席位+入账日期**，两次周检未接线即冻结退回提案区（CPO 修正 1+CTO 具名落点）；**不溯及既往**——已强制法不溯及生效前的 run，评分冻结不追改，实例合规由现行法下新增 run 评分背书（CPO 修正 2）。判例×3：§2.7 节点报告（CTO-F1 证伪 → 校验器+双门）；harvest-rc 匹配器（模拟证伪 → 前 14 位修正）；§8.4-8.6 重构静默丢节（v2.0.0 文本层案例，联审逐行找回）。
...
- **短期**：~~`employee_host_publish` 补齐结构化自检报告~~ ✅（2026-07-24）；~~既有实例补齐试卷与评分~~ ✅（2026-08-20 四实例+08-28 FADE-006 增评）；剩余遗留：FADE-001 服务器侧证据回流、FADE-003 run 链补证、**FADE-003 裁决词表升四态**（v2.0.2 登记册补齐项，期限下次周检）
- **中期**：探索 YAML-based workflow 定义（参照 Conductor），将 Agent 规划层进一步结构化
- **长期**：~~多宿主适配 host adapter~~ ✅（v1.2.0 §6.2）；后续=新宿主注册表条目化扩展与宿主退役机制（agent-governance-alignment-design.md，CEO 定调待办）
