...
定义：**ADE（Agentic Deterministic Execution）是 Agent 全生命周期执行协议，FADE（Full-cycle ADE）即该协议的完整周期实例：采用“智能发现 → 确定性执行 → 智能审核 → CLI 收口”的核心模式，由智能体负责发现与审核的智能环节，由 CLI 负责执行与收口的确定性环节，从而实现固定流程的智能、可靠、可审计与可恢复执行。**
