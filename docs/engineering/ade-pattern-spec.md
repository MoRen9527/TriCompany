...
定义：**ADE 是事件驱动、可恢复、必须收口的 Agent 全生命周期执行协议：以"智能发现 → 确定性执行 → 智能审核 → CLI 收口"的组合，把 Agent 智能执行天然不确定的结果，收敛为按固定流程可靠执行并收口落账的确定结果。核心：按固定流程可靠执行和收口。**

智能负责发现与审核的判断环节，CLI 负责执行与收口的确定性环节；协议的全部机制（runId、状态机、安全门、终态门、恢复与重试）均服务于这一分工。
