...
版本：v1.4.1（2026-08-27：CPO/CTO 联审修后放行——§2.7 校验器强制+§2.8 载体降级合同/判例收口/ validation 齿条）
日期：2026-08-21
状态：当前工程规范
...
定义：**ADE（Agentic Deterministic Execution）是Agent确定性执行的全生命周期（FADE）的协议泛化部分，FADE（Full-cycle ADE）是该协议的完整周期实例实现：采用“智能发现 → 确定性执行 → 智能审核 → CLI 收口”的核心模式，由 Agent 负责发现与审核的智能环节，由 CLI 负责执行与收口的程序环节，从而实现固定流程的智能触发、可靠执行、可审计与可恢复的全生命周期实例规范。**
...
### 2.8 段合同与实现绑定（v1.4.0，六实例反向工程第二次立法）
...
```text
ADE 协议层   ：十段各定义「职责不变量 + 产物合同」——必须满足，与载体无关
FADE 实例层  ：每段选择具体实现载体，入册时声明「段-实现映射表」
```
...
| Qualify | 机械可判定或语义判定留痕 |
| Plan | 结构化计划 + 试卷声明（§2.6） |
| DCE | 确定性、可复现、envelope（§2.2） |
| Verify | （可选）独立于执行者的校验 |
...
**合法载体示例（非穷尽）**——登记段：显式 runId 字符串（journal-cli、staffing）/ manifest 状态条目（发布域）/ jobs.json jobId + per-run 日志（周迁移）/ registry (treeId,tick,pid) 三元组 + hook.log（执行面）。
...
5. 载体显式度分级：**单一显式标识 > 分散组合**——多段复杂实例推荐显式 runId（FADE-002 复评 7→10 实证）；
...
9. **八 profile 限定（CPO-F2 和解）**：确定性拾取门为 runtime-owned/自动触发 profile 的强制不变量；Agent-owned/interactive profile 允许语义判定留痕（§2.8 Qualify 行与 §8 两 profile 的和解口径）。
