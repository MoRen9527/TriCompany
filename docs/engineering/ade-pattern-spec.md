...
- v1.7（2026-08-18）：CTO 联审修订 §一——定义主语回正为 ADE（FADE 保持实例称号）、恢复"事件驱动、可恢复、必须收口"性质定语与 CEO 核心句、删除背景段、术语统一（智能体→Agent）
...
定义：**ADE 是事件驱动、可恢复、必须收口的 Agent 全生命周期执行协议，以"智能发现 → 确定性执行 → 智能审核 → CLI 收口"为核心模式：Agent 负责发现与审核的智能环节，CLI 负责执行与收口的确定性环节，把 Agent 智能执行天然不确定的结果，收敛为按固定流程可靠执行并收口落账的确定结果。核心：按固定流程可靠执行和收口。**
