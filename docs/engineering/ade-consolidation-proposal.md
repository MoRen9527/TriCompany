...
- 发布 CLI 宿主参数（CEO 2026-08-19 定调）：`source_publish_check --publish-agents --host={copilot|claude}`——**先支持两宿主**：`--host=copilot` 发布到 Copilot-host 面（`.github/agents/`，默认值兼容现状）；`--host=claude` 发布到 Claude Code 面（`.claude/agents/`，承接 TriLC init-assemble 装配职责）。每宿主注册三项：渲染模板（frontmatter 形状映射 + 附加段）、live manifest（liveRoot）、保护白名单；标准管线（源+宿主模板→渲染→派生一致校验→审计落账）不变，**未来支持任何宿主 = 新增一个宿主分支（模板+manifest+白名单），管线零改动**
- 双部署模型定调（CEO 2026-08-19）：**宿主侧部署**（Claude Code / Copilot-host）——走多宿主渲染模型，员工能力直接平移宿主侧，**有宿主形态知识工作空间**（knowledge/roles+employees 声明壳与支持对象）；**runtime 侧部署**（TriMC + TriLC）——不渲染，contract 直读，roster.active 即上岗、其余为候选员工，**无宿主形态知识工作空间**。runtime 侧知识载体实证：五件套（静态契约）+ `.tricompany-cognition/`（SQLite 每项目隔离，TRICOMPANY_COGNITION_HOME 实现落点，TriLC multi-project-router 已实现）——缺的是宿主形态知识工作空间的 **runtime 等价物与内容注入**（并入 FADE-ASSESS-003 知识注入消费链路范围）
- 发布链 runtime 侧等价映射（CEO 2026-08-19 确认补录）：宿主发布链 source→support→binding→live→manifest→governance 在 runtime 侧各环节均有替代形态——source=同一源五件套+contract（无差别）；support=五件套契约 + `.tricompany-cognition/` SQLite（employee/org/shared/org/audit 命名空间，binding profile runtimeNamespaces 已定义）；binding=roster.active（在岗绑定）；live= `/agents` API（contract 直读，无渲染文件）；manifest=roster.status（active/pending-cho/candidate 资格三档）；governance=同一套 CHO handoff + CAO governance + CEO 签署（FADE-004 stage 8/10）。差异只在形态：宿主侧=文件资产形态，runtime 侧=运行时状态形态；唯一真缺口= `.tricompany-cognition/` 有存储形态无内容注入链路（FADE-ASSESS-003 核心）
