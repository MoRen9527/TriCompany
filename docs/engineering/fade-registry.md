...
- **实例不完整定性（2026-09-21，CEO 定性；FADE-001 registry owner 落条）**：W38→W39 翻周实例（2026-09-20 23:00，R 面 R-HY TriRMC cron 执行）＝**ADE-only 死工作案例**——有触发（cron 到点）、有 DCE（W39 骨架目录建起），但 **无 agent plan**（动手前不规划：在办树未迁移判定缺/回流状态未查/冲突无预案——旧设计「迁移段序固定无需逐次语义规划」在静态场景成立，遇在办树与三端未回流等动态情境即失效，静态固化≠规划豁免）、**无考卷**（验收卷缺：W39 完整性无断言、在办树随迁无核验）。「只做 ADE 死工作，干活不可靠的根因」（CEO 原话定性）。
- **实例补课·承办件指针（2026-09-21 归一定谳；双写归一 d719c55×acf8748，registry owner 裁）**：补课三件正身——①**agent plan**=`fade-001-plane-shift-agent-plan.md`（触发/到岗窗/D-24 前置断言/五段链＋归一补强两节：在办树逐一点名/台账随迁核验——本席定性三必答已全部承载）；②**考卷**=`fade-papers/FADE-001-paper-plane-shift.json` **1.1-draft**（T1-T9 九题权重卷：sg 原 T1-T6＋归一并入 T7 在办树随迁 20/T8 台账对表 15/T9 新周实质非空 10——在办树题=本次事故盲区正身题；候冻结仪式照 paper-maintenance 卷先例双 hash+authority）；③定性详版=上行条目（本席落条）。~~`fade-papers/FADE-001-paper-shift-v2.json`~~ 已删（一物一卷，三题并入正身卷 T7-T9）。制度面（立规三件套含实例考卷）归 CAO 同步知会在途；本条目=实例层补课。下次翻周 2026-09-27 23:00 前生效。
- 维护项（扩维后）：
...
- 齿条新增③（2026-08-31，LG-016 件 5 落地·周一晨检断言）：**weekly-plane-shift job lastRunStatus=ok 核验**——每周一晨检查 heyuan TriRMC cron job 9c81c7ec 的 lastRunStatus=ok 且 lastError=null（GET /internal/v1/cron/jobs）；非 ok 即迁移失败暴露口（2026-08-30 W35→W36 首跑 PASS 为基准样本：ok/9342ms/rc=1）。配套：rmc-orchestrate-tick（381a1886）同期恢复 ok（08-30 深夜 PATCH 去 runAs，250 连错清零）。
...
## FADE-008 受控压缩（controlled compaction：常驻会话状态污染响应·快照→压缩→重建→核验→转正闭环）

**首跑实例 fade-cos-compact-001（2026-09-16/17；CEO 令二批准并升格 FADE 实例，2026-09-17 00:02+0800 BOD 转达；E 回声=F 铸本条目＋台账回声节）**

| 段 | 工件 |
| --- | --- |
| 触发 | 状态污染实证：两次「声称合入」与实文不符＋窗内累计 ~31 万 token（310k）新旧混杂——CEO 批受控压缩并指示升格 FADE-008 治理闭环标准实例化首跑 |
| 快照（先行硬序） | 全量快照落 `TriMetaverse/.fade/hub-snapshots/full-20260916T164920Z.md`（**411954B**，mtime 2026-09-17 00:49+0800；台账镜像/记事本/常驻指令区/挂账链全含） |
| 迁移独立窗 | 受控压缩移独立可唯一定位窗口执行（BOD harness 代敲授权域内；多 tab 拓扑禁用边界遵行） |
| 压缩 | /compact 执行（独立窗） |
| 重建自证 | SOP 四步全绿（2026-09-17 02:54+0800）：①水位自估=低（重建体归一）②常驻指令区重读（台账尾 40 行——CEO 三条定调/补规细则/E2 裁决/晨检幂等条/M-001/M-004 七族在位）③快照验证（411954B 在盘）④挂账三项复述（B4 扫尾/CSO+DE 收编/FADE 登记）——全量报告发 bod（msg 72fc9521） |
| 董事会核验 | diff 核验 **PASSED**（四点逐一对表：指令区/快照 411954B/挂账项/自报与董事会记录全符）——BOD 转正令（2026-09-17 02:5x+0800），provisional 解除，canonical m-cos 确认 |
| E 回声 | 本条目 F 铸登记＋台账回声节＋可复制性读数（下）＋四工程发现（下）；全程留痕=ledger-mirror.md 各段节 |

**可复制性读数（下次受控压缩照此标准执行）**：
1. **触发判据**：状态污染实证（声称≠实文 ≥2 次）或窗水位异常（>25 万 token 且新旧混杂）；单次不实声称只更正不触发。
2. **快照先行是硬序**：压缩前必须全量快照落盘（`full-<UTC时间戳>.md` 命名，文件名字典序=时间序）；未快照禁压缩。
3. **独立窗执行**：压缩在独立可唯一定位窗口执行（BOD 代敲授权边界：VS Code 多 tab 拓扑禁用）。
4. **重建自证四步**（重建体首轮）：水位自估＋常驻指令区重读＋快照文件验证＋挂账复述——四步齐才报董事会。
5. **董事会 diff 核验**：自证报告与董事会记录逐点对表，PASSED 方解除 provisional 转正。
6. **E 回声双落**：fade-registry 实例条目＋台账回声节同窗完成。
7. **工程坑位四条（首跑实证）＋对策**：①resume 名不自动注册（重建体会话需显式处理命名）②tmux SendKeys 无法输入中文③IME 中文态拦截按键④文本附带 Enter 被 bracketed paste 吞——**对策**：长文本走剪贴板粘贴；命令输入用 EN 态＋补发独立双 Enter＋capture-pane 验空框验收。
8. **全程留痕**：各段时刻＋工件路径＋msg id 入台账镜像；本条目为登记真源。
...
## FADE-010 干活流程全自动化（候批-催办-审批环自动化）——FADE 体系自我应用

> **立项（2026-09-21 CEO 21:09 命题+23:24 加急指正「把催办从纪律变成机制」；00:26 批准登记）**：用 FADE 管线把 FADE 管线自身的候批-催办-审批环自动化——「员工干完活等批示没人管」断链机制化。正身来源=`TriMetaverse/docs/workflow/operating-records/2026-W39/fullauto-loop-synthesis-v2.md`（七席联审合流稿 v2 九节定稿：7/7 段稿+真源三席归口闭环+零分歧零空位）。

| 段 | 实现 |
| --- | --- |
| 事件触发 | 任务进入候批态（执行席收口钩子判定；服务域=自动，本地域=半自动） |
| 登记 | 候批信入 letter-store（`task_ref` 键=任务书路径#节点 id 或 LG-xxx）＋台账同刻挂账（D-27 v2「发生即同步」扩展；发生不同步即违规） |
| Qualify | 收口态含候批判定→收口 notify 与候批信同刻双发 |
| Plan | 候批信模板生成（四行 body 模板槽位每槽≤40 字，零 LLM——R2；信封九字段含 `track` 三轨/`waiting_since` 素材齐备时点/`sla` D-27 v3 双型锚载体/`required=human` 徽标） |
| DCE | notify bod+coo 双投（seats.json 名册解析已就位零新增路由）＋信箱层幂等覆盖；**留痕层只追加**（六+二字段含首写时点/覆盖计数——防覆盖抹首写致候批时长不可算） |
| Verify | 双态对表（信箱 pending 台账无条=漏挂/台账候批信箱无信=漏写/时差>5min=不同步——三型差口即告警源）；闭环判据=task_ref 对表零差口 |
| Score | 衍生指标零成本派生：候批时长（已批时点−首写时点）/催办响应时长/收口周期——入绩效观察列（CHO 酌） |
| Close Skill | CEO 批→信箱 resolved＋台账销账同刻双写→自动回流开工 |
| Close CLI | 催办留痕（六+二字段）=绩效效率源账面（一账两用：流程健康读数+绩效审计） |
| 终态 | 一期服务域跑通一例全环无人值守（sg 值席批次模式制度化扩展到候批/审批环） |

- **真源定谳（三席归口闭环）**：分层双真源——实时态真源=信箱（机器事件流）/治理确认态真源=台账（COS 治理策展）；**跨层唯一通道=COS 核认门**（核认即定谳，防旁路）；COS=确认者非誊抄员。
- **三层催办（时点表定稿）**：①催执行席（hard 到点/normal 24h；三要素+终点）②催审批链（daemon 判定→COS 催 BOD；BOD 提醒 CEO=24h hook 注入三行清单+48h toast 双保险）③催督办面（48h 升 COO→72h 呈 CEO；积压三档：常规同族≥3/阻塞≥2/red 不入档）。超时≠批准；不吵五律；对表三律。
- **人工保留点六条（BOD human in session=「基本自动」边界）**：CEO 呈批转呈/敏感面操作/升级终裁/红线例外批准/走查抽验/治理定调——清单外全自动化。
- **成本红线（CFO R1-R5）**：R1 零巡检原则（判定一律 daemon 确定性代码，严禁定时唤醒 LLM 巡检=唯一翻车形态）；R2 模板化零 token；R3 催办≤1 轮·周 100 万护栏；R4 BOD 摘要≤500 tokens/封；R5 分域计费。全链事件触发 <0.1% 周增量。
- **分期（§8）**：一期首落=letter-store action 三增（submit-approval/approve/urge）+候批信模板+seats.json 补 coo；次落=sg 拾取器 cron 化+超时催办器；二期=本地域三面可见（现役复用 100%）；三期=七席整合+制度正身。
- 规范/正身：合流稿 v2（来源件）+notify-mailbox 载体（LG-036 现役扩展）+`2026-W39/fullauto-loop-cos-p-section.md`（P 段细则）
- owner：COS（主办/催办/核认门）+CTO（载体实现）+BOD（human-in-session）+COO（督办）+CPO（体验）+CFO（成本）+CAO（制度）
- 状态：**登记即活实例**——一期首落三件候技术窗（CTO 排），sg 跑通一例后出评分卷（照 FADE-001 先例补课-考卷-评分链）
