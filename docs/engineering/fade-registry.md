...
- **实例补课·迁移项 Plan 段与考卷（2026-09-21 同批，下次翻周 2026-09-27 23:00 前生效）**：
  - **agent plan（迁移前规划，cron 动手前必产）**：三必答——①搬什么：本周目录清单＋**在办树逐一点名**（open 态树随迁判定：随迁/留置+理由）；②怎么验：三端回流对表序（工作仓↔bare↔GitHub HEAD 逐端）；③冲突预案：回流冲突解法＋README 周指针更新（runbook 既有条款升格为 plan 必答项，非可选参考）。
  - **考卷（迁移后验收卷，全绿方收口）**：①W39 完整性断言（周索引件/daily-progress 存在且非空）；②在办树随迁核验（plan 点名的在办树 100% 到位或留置有据）；③台账对表（LG 现役条目周指针已随迁核验）。考卷落 `fade-papers/FADE-001-paper-shift-v2.json`（卷 ID 延续 FADE-001-paper 族）。
  - 制度面（立规三件套含实例考卷）归 CAO 同步知会在途；本条目=实例层补课。
- 维护项（扩维后）：
