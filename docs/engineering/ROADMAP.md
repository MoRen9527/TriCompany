...
CPO / CTO 对 TriLC 代码审计后追加裁决：完整段链开工 `FREEZE`，先完成 P0 事实基线修复，再推进单项目、单定义、TriLC 单写主的 `runtime-owned-durable` MVP。

实施顺序：

1. 在 `@trimetaverse/agent-core` 落共享 FADE contracts、完整 orchestrator、Skill/DCE/Close runtime 与测试向量。
2. 优先复用 TriLC 已有类 Claude Code Agent loop、SkillTool、permissions、cron、HITL 和本地工具，完成首个 SQLite 本地域 FADE Host。
...
5. 由 TriCompany Trees v0.4 公司协议投影 FADE run，各项目只维护实例 adapter。
...
进入实现前置条件：CEO 确认首期 local-first 范围、完成 TriLC P0 基线、CAO 对 Trees v0.4 公司真源补签，并以 `project-source-doc-sync@1.0.0` 作为首个 FADE definition。
