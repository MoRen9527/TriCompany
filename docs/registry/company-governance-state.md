...
- 经营 owner 为 ChiefAdministrativeOfficer（CAO）；CEOChiefOfStaff 负责路由、协调、公司级分派与升级，兼任会议记录与整理中枢（董事会散议→整理任务书→呈批→董事会令），监督 COO 董事会令执行并为其上下文作主干备份（2026-09-16 CEO 定谳）；收口督办与节奏管理（催办随迁）归 COO，登记收口由 CompanyGovernanceRegistry 执行（2026-09-11 ⑦ 改排，LG-034 批准令+CHO APPROVE 04:15）。
...
| 职责 | Owner |
| --- | --- |
| 模块产品事实与 PRD 归属；产品域收口职责与模块设计/归属方案主责（域内回写 owner；收口触发/时序归 COO 督办、登记归 CGR、呈报归 COS——2026-09-11 五裁① 落格） | CPO（小乔） |
| 模块代码事实与技术门禁 | CTO（小狄） |
| 公司治理、秘书处与行政制度 | CAO |
| 人力资源、岗位启用与交接 | CHO |
| 公司级任务分派、协调与升级；会议记录与整理中枢（监督 COO 履责+上下文主干备份） | CEOChiefOfStaff（小贾） |
...
- COS 监督 COO 系对席履责监督（董事会令执行质量），COO 收口督办系对事监督（收口事项）——对象不同、并存不冲突（2026-09-16 CEO 定谳界说，防双头监督误读）；sg 值班位 COS（m-duty-cos）与本机 COS（m-cos）互备交叉验证，任务不丢。
...
- TriLC 与 TriMC 应消费同一共享 Trees / FADE runtime 合同（协议正身=fade-protocol-spec.md）；本地域和服务域只保留 adapter 差异，并通过 run authority 防止双活写入。
...
- 架构表中的模块一旦被写成正式模块面，默认必须具备以下标配：
  1. 独立 git 仓。
  2. `README.md`。
  3. `docs/` 八件套文档基线（2026-09-11 LG-034 批准令②，取代六件套口径）：`engineering`（技术真源；把关 CTO）、`execution`（分支执行证据；把关 CTO）、`product`（产品真源池化登记；把关 CPO；`product/prd/` 子目录系单版本需求正本，CPO 单签）、`registry`（工作型登记层；登记规则归 CGR，内容 owner=product-state→CPO／code-state→CTO／business-state→BSR／test-state→STE 供料+CTO 门禁收口〔2026-09-11 LG-035 增补：测试状态登记件，与 product-state/code-state 同族；标准段式=当前测试基线/门禁状态/已知缺口/最近验证时点，格式细则归 CAO〕／readme→模块 owner）、`training`（岗位/模块/代码/流程导读；把关 RDT）、`workflow`（模块流程制度与模块级 operating-records；制度面把关 CAO，公司级经营周面唯一承载 `TriMetaverse/docs/workflow/operating-records/` 归 COS 收口）、`contract`（模块对外承诺与接口契约；CTO+CPO 双签）、`testing`（验证面；STE 承接、门禁判定 CTO 收口）。`runs` 不立：模块级运行过程数据属动态运营数据，禁入模块源侧 `docs/`（宿主支撑面/`.tricompany-cognition/**` 承载）；`TriMetaverse/docs/runs/` 存量按历史冻结归档（原位冻结+README 注记行，不再新增）。分级标配：现役核心模块八件套全配；小模块/占位模块六件套为底，`contract`/`testing` 按实质承接时立、占位模块豁免；新建目录必须随首件实质内容落盘，禁空目录。
...
## STE 测试基础设施登记（公司级测试基础设施·R 面承载，LG-035 战略级，2026-09-11）

- 生效：2026-09-11 22:03+0800，CEO 定性「公司与项目两维度最重要记录点之一；元现实反向优化元认知、改造元虚拟最有力的实证之一」；登记执笔=CAO（CGR 经营 owner 面）。
- 定性：**R 面（TriRLC+TriRMC，代码/工具/库全可控+两套智能体班子共用 agent-core）整体纳入 STE 测试工具——R 面 ⊃ 公司级测试基础设施（包含关系非等同）**，公司级测试基础设施系 R 面用途之一（R 面另承载运行面/值班位/代理链等；勘正=CEO 2026-09-11 22:06）。
...
- **Board 非人格席位件发布链落位（2026-09-17 知会登记；2026-09-21 定位重塑同步）**：源侧三件（D2 批准线）+manifest 注册+两面渲染（`.claude/agents/board.md` 等全套）——Board 系可 spawn 非人格席位类型；roster 未登记（无 staffing 换代语义）；契约面归 CGR 治理域（本行即登记）。**定位重塑（2026-09-21 CEO 定谳，b101a31）**：BOD=CEO 与董事意见的集散地（规范表述流转经 COS 链；不做判断——审批权在 CEO；不追真源；非人格组织机构不具员工能力；旧「审批权唯一持有面/权柄四表」口径废止）——九条正身=`source-agents/board/agent-body.agent.md` 定位节；claude-md 分权制节 BOD 定位条款随批（FADE-002 渲染候窗）。COS 定谳入册批（CHO 696d93a）与 Board 立章知会已在本 registry 与秘书处制度现势，索引归档随 GID 下一次触碰顺办。
- **双控制器对照登记（2026-09-17，CEO 面授定性）**：**8713=TriMLC=M 面 daemon**（TriMLC-Watchdog 保活，只认 8713）／**8711=TriRLC=R 面 daemon**（R 面保活机制候建，建成届时只认 8711）——两套并行控制器各守各面，属机制设计非冗余，位不同面；「8711 退役」一说不存在（BOD 前误已勘正）。白皮书侧=附录 B TriMLC/TriRLC 词条修订 1c 补端口与保活句（同日）。
