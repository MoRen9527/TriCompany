...
| 执行原则 | 所有流程化、可重复的工作必须遵循 FADE 完整五段闭环 | 程序触发 → Agent plan skill → CLI 执行 → Agent close → CLI close | `docs/engineering/fade-protocol-spec.md` v2.0.x（前称 ade-pattern-spec） |
...
所有流程化、可重复的工作——构建、测试、发布、巡检、文档同步、审核——必须遵循 **FADE 完整五段闭环**：
...
关联：`docs/engineering/fade-protocol-spec.md` v2.0.x（前称 ade-pattern-spec）
...
| `docs/engineering/fade-protocol-spec.md` v2.0.x（前称 ade-pattern-spec） | 原则三的执行载体 |
