...
> lastSyncedAt: 2026-09-09（D-23 新立：高峰排窗纪律〔CEO 亲定，BOD 令周三 00:0x〕；前批 D-22 catch 吞错禁令）
...
2. **DeepSeek**：周一至周五 9:00-12:00、14:00-18:00 高峰，空闲价=高峰**一半**——同避；
3. 其余时段（含周末全天）=空闲档。
...
| --- | --- | --- | --- |
| 工作日 9:00-12:00 | 平价 | 高峰 | GLM 类可做，DS 类避开 |
| 工作日 14:00-18:00 | **3× 高峰** | 高峰 | **双高峰：大 token 批量禁排（急件例外）** |
| 工作日 18:00-24:00 | 空闲 | 空闲 | **最优批量窗** |
| 夜间 0:00-9:00 | 空闲 | 空闲 | 最优（值守照常） |
| 周末全天 | 空闲 | 空闲 | 最优 |

适用边界：约束对象=**可调度**的大 token 消耗工作（全量测试/批量 spawn/大规模文档生成/模型密集批处理/长会话密集操作）；急件与交互必需不受限；交互位/值班位日常响应照常。

执行注：CFO 会签随批（联合签认先例=密钥治理正身同构）；排程消费方=COS 排程面+各席批量作业自检。
