...
**v9 增补（2026-09-24，五闸精化随批；COO 枢纽转达）——闸 2 扩展与登记完整性条款并入**：

1. **闸 2 扩展（前置对表）**：派工前置对表原两项基础上增第三项——③**查同树同文件在办**（同一树内同一文件已有在办任务时，禁并行开新单——防多树撞文件）；三项对表全过方可发起派工。
2. **五闸精化**：原三闸（防断/防漏/防重）增二——④**立项即登记**（新单立项/任务书落树时同步登记 master-table.json，COS 铸总表自动联动非人工补录；与登记完整性条款合并）⑤**登记完整性**（本日两缺口实证：core-agent 派工未对表+三闸评估立项未登记催办源——立项未登记=催办漏斗首闸缺失）。五闸成套：防断/防漏/防重/防散/防缺。

批准记录：CEO 三闸终裁③随批；COO 枢纽转达；CAO 审校入册（号位裁定=D-27 v9 同条目递增）。
