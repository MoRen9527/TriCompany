...
- 自动化测试（按用例）：路由 **TestEngineer（小柯）**，按 **确定性执行规程（FADE DCE 段）**执行（Agent 选用例 → CLI 执行 `pytest --json-report` → Agent 读报告判断门禁）。细则见 `docs/engineering/ade-pattern-spec.md` §六。
- 自动化部署（按步骤）：路由 **TriDeployer（小布）**，按 **确定性执行规程（FADE DCE 段）**执行（Agent 规划步骤 → CLI 逐步执行 → 每步自检 → Agent 收口）。细则见 `docs/engineering/ade-pattern-spec.md` §六。⚠️ TriDeployer（小布）当前尚未完成上岗流程（无 source-agents 五件套），在 CHO 上岗审批通过前由 CTO 代行。
