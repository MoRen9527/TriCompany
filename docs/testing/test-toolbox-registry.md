...
## 五、四 daemon 测试执行体矩阵（CEO 面授矩阵·STE 域登记 2026-09-14）

| daemon | 角色定位（CEO 面授） | 测试域含义 |
|---|---|---|
| TriMMC（sg，服务域主控） | 元虚拟面主控 | M 面对测发端/服务域侧组件门执行位（LG-036 端A） |
| TriMLC（本机 8713，TriMLC-Watchdog 保活） | 元虚拟面本地控制器 | M 面对测收端+E2E 本机执行体（LG-036 端B） |
| TriRMC（河源，服务域主控） | 现实面主控 | R 面对测（候规划） |
| TriRLC（本机 8711，保活机制候建） | 现实面本地控制器 | R 面 E2E 执行体（候建=测试基建候建项；建成前 E2E 门禁含 R 面 daemon 存活前置断言防假红） |

跨面主控对：TriMMC↔TriMLC（M 面）/ TriRMC↔TriRLC（R 面）/ TriMMC↔TriRMC（跨面）——三面域 E2E 测试矩阵拓扑锚（STE 域消化 2026-09-14）。

## 六、候办与裁决记录

| 项 | 裁定（CTO 2026-09-11 22:55） |
|---|---|
| MCP 浏览器工具 | **提请制不预装**——自动化基座=playwright-core（已登记）；MCP 属交互 agent 在环工具，F1-F5 执行中现真实缺口再呈批，装位按使用面分装（harness 面=用户级 mcpServers；IDE 面=TriPilot 扩展设置），不跨面预装 |
| TriCade 严格 VSCodium 壳 | **同构等价先行**（VS Code+TriPilot 承载 E2E 目的）；严格壳需补打包链→**列 TriCade 打包链批候办（本条即注记，不丢）** |

## 七、R 面测试基础设施（战略级增补，CEO 2026-09-11 22:03 定性；22:06 勘正：R 面 ⊃ 公司级测试基础设施，包含关系非等同——测试基础设施系 R 面用途之一，R 面另承载运行面/值班位/代理链等）

| 维度 | 登记 |
|---|---|
| TriRLC（本机 8711） | 系统测试＋端到端测试执行体——E1-E8 门禁族即首个实战（boot server 范式＋TriRLC key-cache 真链轮询） |
| TriRMC（sg/河源） | 全类型测试编排面（系统/安全/回归/压力——扩展方向） |
| 独占优势四条 | 代码完全可控／工具库完全可控／两套智能体班子／共用 agent-core ⇒ 最完善充分测试能力 |
| 战略定性（CEO 原文） | 2026-09-11：此系元现实反向优化元认知、改造元虚拟最有力的实证之一（测试工具箱台账首册逢其时） |

双维度登记并行：公司维度=TriCompany governance-state（COS 转 CAO/CGR 链执行）；项目维度=TriMetaverse test-state（项目级）。本台账=STE 域第一落点。
