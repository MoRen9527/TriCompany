...
- sourceOfTruth: TriCompany/docs/testing/lg-026-p2-wake-chain-acceptance-report.md
...
- 边界遵守: 只验证未改 TriRLC 源码；TriCompany 仓只读（g1 复证前后 git status 零改动）；移开复跑现场已还原（TriRLC 工作区干净）；报告落 TriCompany/docs/testing/ + 证据 evidence/lg-026-p2/
...
- **门禁④活模型 E2E 结案行（2026-09-02T08:06-08:35Z 四轮，ST 判全链通过）**: 双触发线执行（CEO 备料 07:00Z 触发）。第一轮（占位 token）分型=网关在位+token 未生效；经网关进程内存提取部署 token（procdump 法，CTO 核可入档，dump 即用即删）后第二轮 keys 4 providers 通、暴露**第三型阻塞**=TriModel dist 旧构建吞错；FD triage 三件（C rebuild reason 恢复 / B 案 f5d024e TRILC_LEAD_MODEL env 化 / probe 裁 glm-5.3-flash）后第三轮**模型链全通**（glm-5.3-flash 四轮 STREAM 零 failed 零 fallback，组长 completed 80.9s），新遇**第五型阻塞**=TriRLC allow 规则层 default-deny 拦截（[blocked] not explicitly allowed——清单可见≠执行放行，M12 预警面的 TriRLC 内部规则层实例，ST 漏探教训入档转 P4 门禁：授权面黑盒必列）；FD 整改 03b660d（per-agent permissionRules 注入 ALLOW 五件，落法优于裁案）后**第四轮 12/12 全 PASS**：组长真唤醒→letter_list_pending 过→letter_deliver 真投（status delivered+deliveredAt ISO+Z+台账 send+deliver 两行+deliver actor=组长）→眠（27s 单 turn）+直推帧双段实证（寄信 letter_inbox 活推 R1+真投后 delivered 补拉 R2）。模型 behavior 亮点：第三轮工具被拦后正确推理转通报（systemPrompt 设计有效性实证）。证据 evidence/lg-026-p2/gate4-*.log×7 + gate4-r4-lead-session-messages.json + gate4-live-e2e-final.ts。**门禁④闭，P2 升 PASS**（候 CTO 核后经 COS 报 BOD 全闭）。
...
- 证据九件：`TriCompany/docs/testing/evidence/lg-026-p2/`（endpoints-blackbox.ts+log / mintier-blackbox.ts+log / blindspot.ts+log / fullreg-run1.log / fullreg-run2-moved-aside.log / b5c-fix.ts）
